package ng.canon.downloadgram.Buckets
...
import kotlinx.android.synthetic.main.photo_bucket.*
import kotlinx.android.synthetic.main.save_row.view.*
import ng.canon.downloadgram.GlideApp
import ng.canon.downloadgram.R
import java.io.File
import java.util.*

class PhotoBucktes : AppCompatActivity(),SwipeRefreshLayout.OnRefreshListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.photo_bucket)

        toolbar.title = getString(R.string.save_tabs)
        toolbar.setTitleTextColor(ContextCompat.getColor(this@PhotoBucktes,android.R.color.white))
    ...
    fun FileQuery() {
        ...
        arrayList!!.clear()
        path = Environment.getExternalStorageDirectory().absolutePath + "/downloadgram/"
        val dex = File(Environment.getExternalStorageDirectory().absolutePath, "downloadgram")
        if (!dex.exists())
            dex.mkdirs()
        ...
        observable = Observable.create(object: ObservableOnSubscribe<String> {
            override fun subscribe(subscriber: ObservableEmitter<String>) {
                ...
                cur = contentResolver.query(queryUri, projection, MediaStore.Files.FileColumns.DATA + " LIKE ? AND " + MediaStore.Files.FileColumns.DATA + " NOT LIKE ?", arrayOf(path + "%", path + "%/%"), MediaStore.Files.FileColumns.DATE_ADDED + " desc")
                ...
                if (cur != null) {
                    ...
                    if (cur!!.moveToFirst()) {
                        ...
                        do {
                            ...
                            size = Formatter.formatFileSize(applicationContext, big)
        ...
        observable!!.subscribeOn(Schedulers.io())
                ...
                .subscribe(object: Observer<String> {
                    override fun onComplete() {
                        ...
                        adapter = Photo_Adapter(applicationContext, arrayList!!, this@PhotoBucktes)
...
class Photo_Adapter(var context: Context, var arraylists: ArrayList<Photo_Model>, var downloads: PhotoBucktes) :
    ...
    class Item(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bindData(_data: Photo_Model, _cube: PhotoBucktes) {
